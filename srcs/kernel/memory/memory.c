...
/*   Updated: 2022/03/07 10:30:59 by lubenard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void init_memory(multiboot_info_t *mb_mmap) {
	...
	printd(KERN_INFO, "Kernel is %d bytes, aka %d pages, aka %d page table", get_kernel_size(), get_kernel_size() / 4096, (get_kernel_size() / 4096) / 1024);
	...
	printd(KERN_INFO, "Page directory should begin at %p and end at %p", start_pd, (char *)start_pd + (nframes * sizeof(uint32_t)));
