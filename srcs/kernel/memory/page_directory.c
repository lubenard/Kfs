...
/*   Updated: 2022/01/07 15:36:03 by lubenard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void map_page(void *addr) {
	...
	if (page_directory[pdindex] & (1 << 0)) {
		...
		if (page_table[ptindex] & (1 << 0)) {
			printk(KERN_WARNING, "ptindex present, page already mapped");
			return ;
		} else {
...
void init_pd_and_map_kernel(void *start_addr, uint32_t nframes) {
	...
	if ((uint32_t *)start_addr + nframes >= (uint32_t*)0x3FF000) {

		printk(KERN_INFO, "Test variable 1 = %d", ((char*)0x400000 - (char *)start_addr) / 4);
		printk(KERN_INFO, "Test variable 2 = %d", nframes - (((char*)0x400000 - (char *)start_addr) / 4));
		printk(KERN_INFO, "Test variable 3 = %d", (nframes - (((char*)0x400000 - (char *)start_addr) / 4)) / 1024);

		// Should be enabled
		for (i = 0; i < (int)((nframes - (((char*)0x400000 - (char *)start_addr) / 4)) / 1024) + 1; i++)
